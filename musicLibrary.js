...
var printPlaylists = function () {
  ...
  for (var key in library.playlists) {
    if(library.playlists.hasOwnProperty(key)) {
      var keyHolder = library.playlists[key];
      key += ": " + keyHolder.name;
      key += " - " + keyHolder.tracks.length;
      console.log(key);
    }
  }
...
var printTracks = function () {
  for (var key in library.tracks) {
    if(library.tracks.hasOwnProperty(key)) {
      var keyHolder = library.tracks[key] ;
      key += ": " + keyHolder.name;
      key += " by " + keyHolder.artist;
      key += " (" + keyHolder.album + ")";
    }
    console.log(key);
  }
...
addTrack("king nothing", "Metallica", "King nothing");
